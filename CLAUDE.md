...
- `README.md` — the long one. Setup, every env var, per-page behaviour, the
  full trigger/action/template tables, routes, and the auth model.
- `PRD.md` — the original spec. Build-step numbering in conversation is often
  offset from §8; read what is being asked, not the list.
- `INTEGRATIONS.md` — what is still preview-only (domain registration, Resend
  sending domains, A2P submission, Google/Apple sign-in) and what bit us.
- `AI_AGENTS_WIRING.md` — what remains unwired in Conversation AI. Booking
